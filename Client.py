...
def Image():  # take an image, send size of the image, send image
    camera = PiCamera( #take a picture
        resolution=(1280, 720),  # resolution of image
...
def Video(duration): #do a video, send size of video, send video
    ...
    file_name_video = folder_name_video + \
    file_name_format.format("Video", file_type_video)# save as video.h264
    ...
    while duration_now < duration: #do this till actual duration ist the given duration
